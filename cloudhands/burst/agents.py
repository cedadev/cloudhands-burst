...
def supply_nodes_to_requested_hosts(session):
    log = logging.getLogger("cloudhands.burst.agents")
    with concurrent.futures.ProcessPoolExecutor(max_workers=4) as exctr:
        jobs = {
            exctr.submit(create_node, name=host.name): host for host in hosts(
